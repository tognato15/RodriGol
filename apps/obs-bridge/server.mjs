...
function broadcast(command,source="api"){const envelope={type:"command",sequence:++sequence,source,sentAt:new Date().toISOString(),command};lastCommand=envelope;for(const socket of clients)send(socket,envelope);return envelope;}
...
const server=createServer(async(request,response)=>{
  ...
  if(pathname==="/health"){json(response,200,{status:"ok",app:"@rodrigol/obs-bridge",version:"0.2.0",connections:clients.size,sequence});return;}
  if(pathname==="/api/state"){json(response,200,{connections:clients.size,sequence,lastCommand});return;}
...
server.on("upgrade",(request,socket)=>{
  ...
  clients.add(socket);send(socket,{type:"welcome",version:"0.2.0",sequence,lastCommand});
