...
let tickerRotationTimer=null;let tickerIndex=0;
...
function setCrest(element,side,value={}){const target=element.querySelector(`[data-crest="${side}"]`);if(!target)return;target.style.background=value.background||"#18394a";target.style.color=value.color||"#fff";target.innerHTML=value.image?`<img src="${value.image}" alt="">`:escapeHtml(value.text||"?");}
function setData(element,data={}){for(const [raw,value] of Object.entries(data)){if(raw==="chronology"){renderChronology(value);continue;}if(raw==="homeCrest"){setCrest(element,"home",value);continue;}if(raw==="awayCrest"){setCrest(element,"away",value);continue;}const field=aliases[raw]??raw;const target=element.querySelector(`[data-field="${field}"]`);if(target&&value!==undefined&&value!==null)target.textContent=String(value);}if(element.id==="ticker"){configureTicker(element);startTickerRotation(element,data);}}
function renderChronology(events=[]){const root=document.getElementById("chronology");if(!root)return;if(!Array.isArray(events)||events.length===0){root.innerHTML="<p>Nenhum evento registrado.</p>";return;}root.innerHTML=events.map(event=>`<p><b>${event.minute ?? "—"}'</b> ${event.icon ?? "•"} ${escapeHtml(event.label ?? event.type ?? "Evento")}${event.player?` · ${escapeHtml(event.player)}`:""}</p>`).join("");}
function escapeHtml(value=""){return String(value).replace(/[&<>"']/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[char]));}
function formatTickerItem(item={}){return [item.competition,item.eventType,item.match,item.minute,item.person,item.text].filter(Boolean).join("  •  ");}
function startTickerRotation(element,data={}){clearInterval(tickerRotationTimer);const items=Array.isArray(data.items)?data.items.filter(item=>item&&item.active!==false):[];if(!items.length)return;tickerIndex=0;const track=element.querySelector(".ticker-track");const interval=Math.max(3,Number(data.intervalSeconds)||8)*1000;const render=()=>{if(data.endBehavior==="HIDE_AFTER_CYCLE"&&tickerIndex>=items.length){clearInterval(tickerRotationTimer);tickerRotationTimer=null;hide("ticker");return;}const item=items[tickerIndex%items.length];track.textContent=formatTickerItem(item);tickerIndex+=1;configureTicker(element);};render();tickerRotationTimer=setInterval(render,interval);}
function configureTicker(element){const track=element.querySelector(".ticker-track");if(!track)return;track.classList.remove("is-scrolling");void track.offsetWidth;if(track.scrollWidth>element.querySelector(".ticker-window").clientWidth)track.classList.add("is-scrolling");}
...
function restoreRegions(regions={}){for(const [region,entry] of Object.entries(regions)){if(!components.has(region)||!entry)continue;if(entry.visible)show(region,entry.payload??{});else{update(region,entry.payload??{});hide(region);}}}
function connectBridge(){if(socket&&(socket.readyState===WebSocket.OPEN||socket.readyState===WebSocket.CONNECTING))return;try{socket=new WebSocket(socketUrl());socket.addEventListener("open",()=>{reconnectAttempt=0;setConnection("connected");});socket.addEventListener("message",event=>{try{const message=JSON.parse(event.data);if(message.type==="command")receive(message.command);else if(message.type==="welcome"){restoreRegions(message.regions);if(!message.regions&&message.lastCommand?.command)receive(message.lastCommand.command);}}catch{}});socket.addEventListener("close",scheduleReconnect);socket.addEventListener("error",()=>socket?.close());}catch{scheduleReconnect();}}
