...
function eventLabel(event={}){const raw=String(event.label||event.action||event.type||"INFORMAÇÃO").trim().toUpperCase();const labels={GOAL:"GOL",GOL:"GOL",YELLOW_CARD:"CARTÃO AMARELO",RED_CARD:"CARTÃO VERMELHO",SUBSTITUTION:"SUBSTITUIÇÃO",HALFTIME:"INTERVALO",MATCH_START:"INÍCIO DE JOGO",MATCH_START_UNKNOWN:"INÍCIO DE JOGO",SECOND_HALF:"INÍCIO DO 2º TEMPO",SECOND_HALF_START:"INÍCIO DO 2º TEMPO",RESTART:"INÍCIO DO 2º TEMPO",PENALTY_SCORED:"PÊNALTI CONVERTIDO",PENALTY_MISSED:"PÊNALTI PERDIDO",FINAL:"FIM DE JOGO",END:"FIM DE JOGO"};return labels[raw]||raw.replaceAll("_"," ")}
...
function paintAuxiliaryClocks(){
  ...
  document.querySelectorAll('[data-match-timing]').forEach(el=>{
    ...
    if(clock){
      el.textContent=visual.phase==='FIRST_HALF'?`1ºT ${clock}`:visual.phase==='SECOND_HALF'?`2ºT ${clock}`:visual.phase==='EXTRA_TIME_FIRST_HALF'?`1ºP ${clock}`:visual.phase==='EXTRA_TIME_SECOND_HALF'?`2ºP ${clock}`:visual.phase==='EXTRA_TIME'?`PRORR. ${clock}`:clock;
      return;
    }
  ...
  document.querySelectorAll('[data-side-lower-status]').forEach(el=>{
    ...
    if(clock)el.textContent=visual.phase==='FIRST_HALF'?`1ºT ${clock}`:visual.phase==='SECOND_HALF'?`2ºT ${clock}`:visual.phase==='EXTRA_TIME_FIRST_HALF'?`1ºP ${clock}`:visual.phase==='EXTRA_TIME_SECOND_HALF'?`2ºP ${clock}`:visual.phase==='EXTRA_TIME'?`PRORR. ${clock}`:clock;
...
function roundTimingLabel(m,status,beforeKickoff){
  ...
  if(clock){
    ...
    if(phase==="EXTRA_TIME_FIRST_HALF")return `1ºP ${clock}`;
    if(phase==="EXTRA_TIME_HALFTIME")return `INTERVALO PRORR.`;
    if(phase==="EXTRA_TIME_SECOND_HALF")return `2ºP ${clock}`;
