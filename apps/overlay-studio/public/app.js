...
function renderSideLower(){const cards=Array.isArray(sideLower.cards)?sideLower.cards:[],content=$('sideLowerContent');if(!content)return;if(!cards.length){$('sideLowerTitle').textContent='DESTAQUES';$('sideLowerCounter').textContent='0/0';content.innerHTML='<div class="side-lower-empty">Nenhum card selecionado</div>';document.querySelectorAll('.side-lower-dot').forEach(d=>d.classList.remove('active'));return;}const card=cards[sideLowerIndex%cards.length];$('sideLowerCounter').textContent=`${sideLowerIndex%cards.length+1}/${cards.length}`;if(card.type==='lineup'){$('sideLowerTitle').textContent=`ESCALAÇÃO · ${card.teamName||'EQUIPE'}`;const starters=(Array.isArray(card.starters)?card.starters:[]).slice(0,11);content.innerHTML=`<article class="side-lineup-card"><div class="side-lineup-team">${crestMarkup(card.crest,card.teamShort||'') }<strong>${escapeHtml(card.teamName||'EQUIPE')}</strong></div><ol>${starters.length?starters.map(player=>{const text=String(player||'').trim();const converted=text.replace(/^(\d{1,3})[.\s-]+(.+?)(?:\s+\((\d{1,3})[.\s-]+(.+)\))?$/,(_,number,name,outNumber,outName)=>outNumber?`${number} - ${name} (${outNumber} - ${outName})`:`${number} - ${name}`);return `<li>${escapeHtml(converted)}</li>`}).join(''):'<li>Escalação não informada</li>'}</ol>${card.coach?`<small>TÉC. ${escapeHtml(card.coach)}</small>`:''}</article>`;}else{$('sideLowerTitle').textContent=card.competition||'JOGO';syncAuxiliaryClock(card);const status=lowerMatchStatus(card),homeGoals=Array.isArray(card.homeGoals)?card.homeGoals:[],awayGoals=Array.isArray(card.awayGoals)?card.awayGoals:[],hasGoals=homeGoals.length||awayGoals.length,goalsHtml=hasGoals?`<div class="side-score-goals"><div>${homeGoals.map(goal=>`<span><b>${escapeHtml(goal.minute||'')}</b> ${escapeHtml(goal.player||'Gol')}</span>`).join('')}</div><div>${awayGoals.map(goal=>`<span><b>${escapeHtml(goal.minute||'')}</b> ${escapeHtml(goal.player||'Gol')}</span>`).join('')}</div></div>`:'';content.innerHTML=`<article class="side-score-card ${hasGoals?'has-goals':''}"><div class="side-score-status" data-side-lower-status="${escapeHtml(card.matchId||'')}">${escapeHtml(status)}</div><div class="side-score-main"><div>${crestMarkup(card.homeCrest,'MAN')}<span>${escapeHtml(card.homeName||'Mandante')}</span></div><strong>${card.beforeKickoff?'×':`${Number(card.homeScore)||0} × ${Number(card.awayScore)||0}`}</strong><div>${crestMarkup(card.awayCrest,'VIS')}<span>${escapeHtml(card.awayName||'Visitante')}</span></div></div>${goalsHtml}</article>`;}document.querySelectorAll('.side-lower-dot').forEach((dot,index)=>dot.classList.toggle('active',index===sideLowerIndex%Math.min(3,cards.length)));}
